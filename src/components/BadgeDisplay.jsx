    ...
    <div className="bg-white bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] backdrop-blur-sm rounded-2xl shadow-sm p-5 border border-white/50 h-full flex flex-col">
      ...
      <div className="flex flex-col gap-3 flex-1 justify-around">
        {coreBadges.map((badge) => {
          const isUnlocked = badges.some((b) =>
            typeof b === "string"
              ? b.toLowerCase() === badge.id
              : b.id === badge.id
          );
          return (
            <motion.div
              ...
              className={`relative p-3 rounded-xl flex items-center gap-4 transition-all ${
                isUnlocked
                  ? `bg-gradient-to-r ${badge.color} border shadow-sm`
