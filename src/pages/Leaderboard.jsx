      ...
      <div className="max-w-4xl w-full">
        ...
        <div className="flex justify-center items-end gap-4 mb-12 h-64 md:h-80">
          ...
          <PodiumStep
            ...
            icon={<Medal size={32} className="text-gray-500" />}
          />
          {/* 1st Place */}
          <PodiumStep
            rank={1}
            ...
            icon={<Crown size={40} className="text-yellow-600" />}
            isFirst
          />
          {/* 3rd Place */}
          <PodiumStep
...
function PodiumStep({
  ...
  return (
    ...
    >
      <div className="mb-3 text-center relative">
        ...
        <div
          className={`w-16 h-16 md:w-20 md:h-20 rounded-full border-4 ${
            isFirst ? "border-yellow-400" : "border-white"
          } shadow-lg overflow-hidden bg-white flex items-center justify-center text-3xl md:text-4xl`}
        >
          {user.avatar}
        </div>
        <p
      ...
      <div
        className={`w-full ${height} bg-gradient-to-t ${color} rounded-t-2xl shadow-inner flex items-start justify-center pt-4`}
      >
        <span
          className={`text-2xl md:text-4xl font-black ${
            isFirst ? "text-yellow-700 opacity-50" : "text-gray-600 opacity-30"
          }`}
        >
          {rank}
        </span>
...
function LeaderboardItem({ rank, learner, isCurrentUser }) {
  return (
    ...
    >
      ...
      <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-xl shadow-sm mx-4">
        {learner.avatar}
      </div>
      <div className="flex-1">
        <p className="font-bold">
          {learner.name} {isCurrentUser && "(You)"}
        </p>
      </div>
