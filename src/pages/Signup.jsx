    ...
    try {
      ...
      dispatch(
        setUser({
          email: user.email,
          uid: user.uid,
          displayName: user.displayName || "New Gamer",
        })
      );
      navigate("/dashboard");
    } catch (error) {
      ...
      >
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className={`w-full py-4 px-6 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold text-lg rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${
